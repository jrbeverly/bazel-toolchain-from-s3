...
def _register_external_toolchain_impl(repository_ctx):
    ...
    repository_ctx.file(
        "BUILD.bazel",
        """load("@bazel-external-toolchain-rules//bazel/macros:s3_toolchain.bzl", "externally_managed_toolchain")
...
""".format(rules = "\n".join(toolchain_rules)),
    ...
    repository_ctx.file(
        "deps.bzl",
        """load("@bazel-external-toolchain-rules//bazel/macros:s3_archive.bzl", "s3_archive")
